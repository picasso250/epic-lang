...
- `v0` 是可演进的 bootstrap 分支，不是不可移动标签；它必须通过自身 fixed point 与已提交 SHA-256。`v0` 只实现构建当前 `dev` 所需的最小源码语义，不承诺当前公开 ABI：例如它支持只读 `s[i]` 以编译新 frontend，但保留旧 `str`/slice bootstrap 布局。当前 v0 已支持 `embed "path"`；`>>` / `>>=` 按左值类型选择算术或逻辑右移，所有 shift count 必须是 `i64`。当前 `dev` 对裸整数字面量静态检查并消除检查，`v0` 为简化 bootstrap 对所有 count 一律生成运行时检查；`>>>` / `>>>=` 已删除。
