...
- `v0` 是可演进的 bootstrap 分支，不是不可移动标签。它必须保持 `bootstrap/` 与 `src/` 的语言语义一致，并通过自身 fixed point 与已提交 SHA-256。当前 v0 已支持 `embed "path"`；`>>` / `>>=` 按左值类型选择算术或逻辑右移，所有 shift count 必须是 `i64`。当前 `dev` 对裸整数字面量静态检查并消除检查，`v0` 为简化 bootstrap 对所有 count 一律生成运行时检查；`>>>` / `>>>=` 已删除。
