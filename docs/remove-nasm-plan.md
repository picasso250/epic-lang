...
- MIR 采用现代 LLVM-like opaque pointer 思路：pointer value 类型是 `ptr`，内存访问类型由 `load/store/gep` 指令携带。
- MIR 不保留 `struct.new`、`field.load`、`array.push`、`adt.payload` 这类 AST 级便捷 op；结构体/数组/ADT 操作要分解成 `gep`、`load/store`、显式控制流和普通 runtime call。
...
- heap 分配是普通 allocator call；struct 大小和字段地址通过 LLVM-like `gep` / `ptrtoint` / `load` / `store` 表达。
...
- 把语义操作分解到目标 MIR op。禁止把 `struct.new`、`field.load`、`array.push`、`adt.payload` 这类便捷语义直接固化为 MIR op。
- 把 x64 指令映射到 LowMIR / encoder 任务。
- 标出 MIR lowering 必须处理的 ABI 和栈布局规则。

验收：

- MIR op 清单覆盖当前 examples 和 bootstrap，并保持 LLVM-like 低层粒度。
...
- 实现 `add/sub/icmp/load/store/gep/ptrtoint/call/ret` 的最小 lowering。
...
- 每覆盖一类特性，优先复用现有 LLVM-like MIR op；确实需要新 op 时必须先证明不能由 `gep/load/store/call/branch` 简洁表达。
