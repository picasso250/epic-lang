...
- MIR 不保留 `struct.new`、`field.load`、`array.push` 这类 AST 级便捷 op；结构体/数组操作要分解成 `gep`、`load/store`、显式控制流和普通 runtime call。
...
- 把语义操作分解到目标 MIR op。禁止把 `struct.new`、`field.load`、`array.push` 这类便捷语义直接固化为 MIR op。
