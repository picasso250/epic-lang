...
MIR runtime helper body 统一提交在 `runtime/mir/helpers.mir`，Python reference compiler 和 Epic 自举编译器都读取这个 bundle；更新 builder-backed helper 后运行 `python scripts/write_mir_runtime_bundle.py`。旧 `runtime/*.asm` 路线已删除。
