...
运行时统一表示为普通 MIR function。基础 helper body 提交在 `runtime/mir/helpers.mir`，GC 在 `runtime/mir/gc.mir`；array、panic、string 和 file helper 都作为标准 Epic source 与用户程序一起 parse/sema/lower。`src/runtime_bundle.ep` 将这些文件嵌入编译器；x64 后端不附加手写运行时。
