...
heap 分配，地址在生命周期内不移动。runtime 维护紧凑的 payload 基址和请求大小数组；
正常分配路径只执行 HeapAlloc 和记录追加，不给每个对象增加 GC header。
...
- collection 保持一个已排序 survivor 前缀，只对上轮 collection 后追加的尾部做原地排序；
  lookup 先以历史 `low_addr/high_addr` 快速拒绝，再分别二分查找两个有序区间。
  collection 临时建立 mark byte table、迭代 work stack 和线性归并缓冲区。
...
- sweep 以线性归并顺序释放未标记 payload，把两个有序区间合并为下一轮的 survivor
  前缀；对象大小直接读取旁路记录，不调用 `HeapSize`。
