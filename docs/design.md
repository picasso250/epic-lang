...
当前活跃验收入口包括 `python tests/run.py`、`python tests/examples/run.py` 和 `python bootstrap_fixed_point.py`。模块级测试负责 Python/self-hosted 各阶段对拍与 e2e 行为；examples 验证面向用户的正向程序；不动点构建检查 Python 直接生成的编译器与第一次 Epic 自举产物字节一致。
