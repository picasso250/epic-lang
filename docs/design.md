...
当前 Epic compiler 的 `i32/u32/u8` 布局使用 8 字节槽。
...
- self-hosted compiler 的模块划分、文件结构和中间数据结构；
- 性能、内存占用和未显式承诺的优化结果。

只要公开语义和可观察行为保持一致，内部表示可以重构。任何有意破坏公开边界的改动必须在同一提交中更新设计文档和意图级测试，不能依赖旧行为的隐式兼容。
