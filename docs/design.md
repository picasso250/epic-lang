...
位运算符 `~`、`&`、`|`、`^` 和移位运算符 `<<`、`>>` 是低层操作，不经过检查。`>>` 对有符号整数是算术移位，对无符号整数是逻辑移位。需要对有符号值做逻辑右移时，先显式转换为同宽无符号类型。
...
支持：`+=`、`-=`、`*=`、`/=`、`%=`、`<<=`、`>>=`、`&=`、`|=`、`^=`。赋值目标与普通赋值相同：局部变量、struct 字段或数组下标。定位左值所需的对象、数组和下标表达式按源码顺序各求值一次；随后读取目标旧值，再求值右侧表达式，执行运算并写回同一目标。窄整数结果在写回前按目标类型截断并符号扩展或零扩展。复合赋值只用于左右类型完全相同的整数；`str += str` 不支持，需要显式写 `s = s + rhs`。
