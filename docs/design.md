...
当前活跃验收入口包括 `python tests/run.py`、`python test_examples.py` 和 `python test_bootstrap_fixed_point.py`。模块级测试负责 Python/self-hosted 各阶段对拍与 e2e 行为；examples 验证面向用户的正向程序；不动点测试连续使用生成的 Epic 编译器重新编译自身，并检查后续世代输出稳定。
