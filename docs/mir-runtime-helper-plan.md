...
| numeric string helpers | migrated to `runtime/str.ep` (`__ep_str_from_i64`, `__ep_str_from_u64`) |
...
- **x64 primitive** = x64 function/data that requires Windows API knowledge or x64-specific setup. These stay in the x64 backend.
  ...
  2. **x64-internal primitive/data/startup** — not exposed to MIR at all.
     E.g. `_heap`, `_argv`, `__epic_runtime_start`, `argv_init`.
...
This is the **core migration**: instead of emitting x64 asm directly from `mir_lower.py`, helper bodies move into Epic runtime source or MIR `MirFunction` objects that go through the standard MIR→x64 lowering.
...
After Phase 4:
- Numeric string helpers no longer live in `mir_lower.py`; `__ep_str_from_i64` / `__ep_str_from_u64` are implemented in `runtime/str.ep`.
