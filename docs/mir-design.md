...
| Runtime helper impl | `MirFunction` 注入，走正常 MIR→X64 lowering | ✅ MIR bundles 直接加载；array/panic 在 helpers bundle；string/file 由标准 Epic runtime source 合并后注入 |
...
declare i64 @__ep_import$kernel32.dll$WriteFile(i64, i64, i64, i64, i64)
...
- Windows API 导入使用自描述符号 `__ep_import$<dll>$<symbol>`，由 module symbol table 声明。
- call 处引用同一个编码符号；linker 从中解码 DLL 和精确导出名。
...
规范文本 MIR 使用 `declare` 声明。Windows API 与源码 FFI 都使用自描述符号名 `__ep_import$<dll>$<symbol>`，把 DLL metadata 带入 COFF：

```text
declare void @__ep_import$kernel32.dll$ExitProcess(i64)
declare ptr @__ep_alloc(i64)
```

每个 call 必须解析为模块内 `define` 或同模块 `declare`，通用 validator 负责检查完整签名。
serializer 只输出实际被 call 引用的 declaration，并按名称排序；缺失声明立即报错。
旧 `extern` 和 `import` 都不是 canonical MIR 语法。
...
runtime 注入和 unreachable pruning 完成后，backend 会再次裁剪未引用 declaration。runtime bundle 之间的普通 declaration 必须由注入后的 MIR `define` 满足；最终保留的 `__ep_import$...` declaration 作为 Windows x64 导入进入 COFF，由自带 linker 解码 DLL 与精确符号名。后端不维护 WinAPI 名称或签名白名单。
