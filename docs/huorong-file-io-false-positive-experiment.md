...
当时火绒设置中存在覆盖整个项目根目录的排除项：

```text
C:\Users\MECHREV\projects\
```

排除类型为“不扫描指定程序的动作”。实时监控仍开启，扫描时机为文件执行时，处理方式为“询问我”。因此，项目目录内没有弹窗可能只是路径排除或信任策略的结果，而不是文件本身未命中规则。

## 路径隔离验证

为了排除“火绒按路径信任，而不是专用 helper 真正规避检测”的可能性，将专用版本的 `m11_file.exe` 原样复制到项目目录之外的随机 `%TEMP%` 目录，并改成新文件名：
...
本次实验不能证明专用 X64 helper 能规避火绒检测。相反，在排除目录之外测试后，专用版本仍然被火绒报告为病毒。
...
1. 火绒的允许/排除行为不是只依赖文件 SHA-256。
2. 文件所在路径会显著影响火绒是否检查、报警或阻止执行。
3. `C:\Users\MECHREV\projects\` 下的结果不能用于判断一个新 PE 是否命中规则。
...
2. 待测 EXE 必须复制到火绒排除范围之外的全新随机目录。
3. 使用新的文件名，避免继承文件名相关信任。
4. 复制前后核对 SHA-256，确保只有路径和文件名变化。
5. 每个新样本只执行一次，记录火绒界面结果、退出码、输出文件和文件是否仍存在。
6. 不以项目目录内正常执行作为“未命中”证据。
7. 不以火绒 SQLite 日志没有即时记录作为“未命中”证据。
8. A/B 样本应分别放入不同的新目录，避免一次允许操作污染后续测试。
