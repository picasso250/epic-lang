...
Epic v0 引入 `Vec`——基于 `new i64[n]` 的可扩容数组。不设泛型，一 Vec 通吃：

```
struct Vec {
    data: &i64;   // offset 0，指向 new i64[cap] 后备数组
    len: i64;     // offset 8，已用元素数
    cap: i64;     // offset 16，容量
}
```

**设计要点：**

- 后备数组用 `new i64[n]` 分配（flat 8 字节槽）。
- 元素总是存 8 字节（指针或 i64），指针位为 i64 存入，读回时用 `let x: &TargetType = vec.data[i]` 恢复类型。
- 扩容策略：`cap * 2`，新 `new i64[cap*2]`，for 循环拷贝，旧数组泄漏不回收。
- v0 生命周期内不提供 `free`，一次编译器运行泄漏总量 ~MB 级别，OS 退出回收。

**API：**

| 函数 | 签名 | 说明 |
|------|------|------|
| `vec_push` | `vec_push(v: &Vec, elem: i64) → void` | 追加；满则扩容 2x + 拷贝 |

不提供 `vec_pop`、`vec_get` 等——直接 `v.data[i]` 下标访问即可。

**典型用法：**

```epic
let tokens = new Vec;
tokens.data = new i64[16];  // 或封装为 vec_new(16)
tokens.len = 0;
tokens.cap = 16;

let t = new Token;
t.kind = "ID";
vec_push(tokens, t);  // &Token 以 i64 位存入

let back: &Token = tokens.data[0];  // 读回，标注类型
```

**不设 Vec_pop / Vec_get 等：** 直接 `v.data[i]` 下标，类型标注由调用方负责。Vec 只管理容量。
