...
- `../v2` compiles compiler sources with the v1 compiler and uses `build/link.exe`.
...
- Python 3
- `tools/nasm.exe`
- `tools/lld-link.exe` when using historical linker comparison tools
...
Then run the example suite with the previous Epic compiler and Epic linker
anchors:
...
The test runner builds the current compiler with `build\v1.exe`, then uses the
...
- binary byte-buffer support as groundwork for the Epic linker
