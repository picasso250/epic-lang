...
The v2 directory uses the v1 compiler as its previous compiler anchor. Build the
v0 fixed point compiler from `../v0`, copy `epic-epic-epic.exe` to root
`build\v0.exe`, use it in `../v1` to build `v1\build\epic\epic.ep.exe`, then
copy that executable to root `build\v1.exe`.
